...
class Employee extends CI_Controller {
    ...
    public function Delete_employee()
    {
        ...
        $success=$this->employee_model->Delete_employee($id);
        if($success)
        redirect('employee/Employees');
    ...
    public function sales(){
        ...
        if ($this->session->userdata('user_login_access') == 1){
            ...
            $data=array();
            $data['getbasic'] = $this->employee_model->GetBasic($id);
            $this->session->set_flashdata('in',$data);
            redirect('https://sales.thetechcubes.in/');
    ...
    public function Assign_leave(){
        ...
        $type =(int) $this->input->post('typeid');

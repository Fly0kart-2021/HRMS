...
class Leave extends CI_Controller
{
    ...
    public function Application()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->session->userdata('user_type') == 'ADMIN')
                $data['employee']    = $this->employee_model->emselectforadmin();
            ...
            if ($this->session->userdata('user_type') == 'ADMIN')
                $data['application'] = $this->leave_model->GetAllEmployeeLeaves();
    ...
    public function Add_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $appstartdate = $this->input->post('startdate');
            $appenddate   = $this->input->post('enddate');
            $leftdays       = $this->input->post('leftdays');
           
            $checkfuturedate=array();
            $checkfuturedate=explode('-',$appstartdate);

            if($checkfuturedate[1]>date('m')){
                echo "Can not apply for future months";
                die();
            }
            ...
            if ($type == 'Half Day') {
                ...
                $date1= date_create("$appstartdate"); $date2=date_create("$appenddate");
                $datediff =date_diff($date1,$date2);
                $datediff=$datediff->format("%a");
                if($datediff>$leftdays){
                    echo "Can not apply more than allowed days";
                    die();
                }
                $formattedStart = new DateTime($appstartdate);
                $formattedEnd = new DateTime($appenddate);

                $duration = $formattedStart->diff($formattedEnd)->format("%d")+1;
                $duration = $duration * 8;
            }
    ...
    public function LeaveAssign()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if (!empty($leaveID)&& $leaveID==8) {
                $totalleaves = $this->leave_model->gettotalleave($leaveID);
                $totalleaves = $totalleaves->leave_day;
                $joiningdate = $this->leave_model->getjoiningdate($employeeID);
                $joiningdate = $joiningdate->em_joining_date;
                $joiningarray = array();
                $joiningarray = explode('-', $joiningdate);
               
                $year  = date('Y');
                $month = $joiningarray[1];
                $date = $joiningarray[2];
                $fromdate = date_create($year . '-' . $month . '-' . $date);


                $todate = new datetime(date('Y').'-'.date('m').'-1');
                $todate=date_format($todate,'Y-m-d');
                $todate=date('Y-m-d',strtotime($todate.'+1 month'));
                $todate=date('Y-m-d',strtotime($todate.'-1 day'));
                $todate= new DateTime($todate);
                $mdiff = date_format($todate, 'm') - (int)date_format($fromdate, 'm') + 1;
                $fromdate = date_format($fromdate, 'Y-m-d');
                $todate = date_format($todate, 'Y-m-d');
                $takenleaves = $this->leave_model->leavestaken($fromdate, $todate, $employeeID, $leaveID);
                // $takenleaves=$takenleaves->totalleave;
                $takendays= (float)$takenleaves->totalleave/8;

                echo json_encode(array('msg' => $mdiff, 'available' => ((float)$mdiff - $takendays)));
                ...
                $totalleaves = $this->leave_model->gettotalleave($leaveID);
                $totalleaves = $totalleaves->leave_day;
                $totalleaves=round($totalleaves/12);
                $fromdate=new datetime(date('Y').'-'.date('m').'-01');
                $fromdate=date_format($fromdate,'Y-m-d');
                ...
                echo json_encode(array('msg' => $totalleaves, 'available' => ((float)$totalleaves - $takendays)));

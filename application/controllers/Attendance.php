...
class Attendance extends CI_Controller
{
    ...
    public function timeout()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $em_id = $_POST['id'];
            $regular = date('09:00:00');
            
            $out= date('H:i:s');
            $signin= new DATETIME(date('Y-m-d')." ".$_POST['signin']);
            $signout = new DATETIME(date('Y-m-d')." ".$out);
             $working_time = $signout->diff($signin);
             $working_time=$working_time->format("%H:%I:%S");

             $new_Regular = new DATETIME(date('Y-m-d')." ".$regular);
             $new_working = new DATETIME(date('Y-m-d')." ".$working_time);

             if($regular == $working_time ){
                $overtime = 0;
                $absence=0;
                 }

             if($regular < $working_time ){
            $overtime= $new_working->diff($new_Regular);
            $overtime=$overtime->format("%H:%I:%S");
             }
             else{
                 $overtime = 0;
                 $absence =$new_Regular->diff($new_working);
                 $absence=$absence->format("%H:%I:%S");
             }
            ...
            $success = $this->attendance_model->timeout($em_id,$data);

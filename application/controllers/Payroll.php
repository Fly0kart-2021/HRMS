    ...
    class Payroll extends CI_Controller
    {
        ...
        public function Invoice()
        {
            if ($this->session->userdata('user_login_access') != False) {
                ...
                // Count sunday
                $sundays = $this->count_sunday($month, $year);



                $month_holiday_count = $this->payroll_model->getNumberOfHolidays($month, $year);

                // Total holidays and sunday count
                $total_days_off = $sundays + $month_holiday_count->total_days;
        ...
        <div class='col-md-12'>
                ...
                echo "</td>
                                        ...
                                        <tfoot class='tfoot-light'>
                                            <tr>
                                                <th>Total</th>
                                                <th class='text-right'>" . $total_add = $salary_info->basic + $salary_info->medical + $salary_info->house_rent + $addition[0]->conveyance+$salary_info->bonus;
                round($total_add, 2);
                echo "INR</th>
                ...
                echo "INR</th>
                                            ...
                                            <tr>
                                                ...
                                                <th class='text-right'>$salary_info->total_pay  INR</th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
        ...
        private function count_sunday($month, $year)
        {
            $sundays = 0;
        ...
        public function GetSalaryByWorkdays()
        {
            ...
            if ($this->session->userdata('user_login_access') != False) {
                ...
                // Count sunday
                $sundays = $this->count_sunday($monthName, $year);
        ...
        public function pay_salary_add_record()
        {
            if ($this->session->userdata('user_login_access') != False) {
                ...
                $additionbasic = $getaddition[0]->basic;
                $additionmedical = $getaddition[0]->medical;
                $additionrent = $getaddition[0]->house_rent;

                $getdeduction = $this->payroll_model->getdeduction($emid);
                $provident_fund = $getdeduction[0]->provident_fund;
                $bima = $getdeduction[0]->bima;
                $tax = $getdeduction[0]->tax;
                $others = $getdeduction[0]->others;
                $deductiontotal = $provident_fund + $bima + $tax + $others;
                ...
                $data = array(
                    ...
                    'basic' => $additionbasic,
                    'medical' =>  $additionmedical,
                    'house_rent' => $additionrent,
                    'loan' => $loan,
                    'provident_fund' => $provident_fund,
                    'bima' => $bima,
                    'tax' => $tax,

                    'total_pay' => round($total_paid-$deductiontotal, 2),
                    'addition' => $addition,
                    'diduction' => $deductiontotal,
                    'status' => $status,
                    'paid_type' => $paid_type,
                );
        ...
        public function generate_payroll_for_each_employee($call = null)
        {
            ...
            if ($this->session->userdata('user_login_access') != False) {
                ...
                $total_work_days = $total_days_in_the_month;
                ...
                //Get his monthly salary
              $getsalarydetail=  $this->payroll_model->getsalarypaydetail($employeeID);
              $employee_actually_worked = (int)$getsalarydetail[0]->total_days;
                ...
                $work_hour_diff = abs($total_work_hours) - abs($employee_actually_worked); // 96 - 16 = 80
        ...
        public function finalpaytodept()
        {
            if ($this->session->userdata('user_login_access') != False) {
                $depid = $_POST['id'];
            $date = $_POST['datetime'];
        ...
        public function Process_payroll_by_dep()
        {
            # code...
            $depid = $_POST['id'];
            $date = $_POST['datetime'];
            ...
            $employees = $this->payroll_model->GetDepEmployee($depid);

            foreach ($employees as $employee) {
                $_GET['month'] = $datetime[0];
                $_GET['year'] = $datetime[1];

                $_GET['employeeID'] = $employee->em_id;
                $pay_salary_data = $this->generate_payroll_for_each_employee(1);
                ...
                for ($i = 1; $i <= $totaldays; $i++) {
                    $tempdate = date_format(date_create($_GET['year'] . '-' . $_GET['month'] . '-' . $i), 'Y-m-d');
                    foreach ($holidaylist as $holiday) {
                        if ($tempdate == $holiday->from_date || ($tempdate <= $holiday->to_date && $tempdate > $holiday->from_date)) {
                            $finalwh += date('08:00:00');
                        }
                    }
                    foreach ($paidleavelist as $paidleave) {
                        if ($tempdate == $paidleave->start_date || ($tempdate <= $paidleave->end_date && $tempdate > $paidleave->start_date)) {
                            $finalwh = $finalwh + (int)$paidleave->leave_duration;
                        }
                    }
                    foreach ($workinghour as $wh) {
                        if ($tempdate == $wh->atten_date) {
                            if ($wh->working_hour > date('08:00:00')) {
                                $finalwh += date('08:00:00');
                            } else {
                                $hr = array();
                                $hr = explode(':', $wh->working_hour);
                                $time = $hr[0] . '.' . $hr[1];
                                $finalwh = $finalwh + (float)$time;
                            }
                        }
                    }
                }
                $sundays = $this->count_sunday($_GET['month'], $_GET['year']);
                $finalwh += $sundays * 8;
                ...
                $pay_salary_data['employee_actually_worked'] = $finalwh;
                $totalhours = $pay_salary_data['total_work_hours'];
                $totalworkingdays = ($totalhours / 8);
                $salaryperday = $pay_salary_data['basic_salary'] / $totalworkingdays;
                $final_salry = ($finalwh / 8) * $salaryperday;

                $pay_salary_data['final_salary'] = $final_salry;

                $deduction = $pay_salary_data['basic_salary'] - $final_salry;

                $pay_salary_data['diduction'] =  $deduction;

                $status = 'Process';
                $salary_type = 'Bank';
                $paymonth = $_GET['month'] + 1;
                $paydate = date_format(date_create($_GET['year'] . '-' . $paymonth . '-10'), 'Y-m-d');
                $pay_salary_data['status'] = $status;
                $pay_salary_data['paydate'] = $paydate;

                $pay_salary_data['paid_type'] = $salary_type;
                $pay_salary_data['emid'] =  $_GET['employeeID'];
                $pay_salary_data['month'] =  $_GET['month'];
                $pay_salary_data['basic'] =  $pay_salary_data['basic_salary'];
                $pay_salary_data['year'] =  $_GET['year'];
                $pay_salary_data['hours_worked'] = $pay_salary_data['employee_actually_worked'];
                $pay_salary_data['total_paid'] =  $pay_salary_data['final_salary'];
                $pay_salary_data['call'] = 1;
                ...
                // $workinghour=$workinghour[0]->working_hour;
                $_POST = $pay_salary_data;
                $this->pay_salary_add_record();
            }
        }
    }

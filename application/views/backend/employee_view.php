                                        ...
                                        <div class="col-md-8">
                                            <form class="row" action="Update" method="post" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group col-md-4 m-t-10">
                                                    ...
                                                    <input type="text" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> class="form-control" placeholder="NID Number" name="nid" value="<?php echo $basic->em_nid; ?>" minlength="10">
                                                </div>
                                                <div class="form-group col-md-4 m-t-10">
                                ...
                                <div class="card-body">
                                    ...
                                    <form class="row" action="Parmanent_Address" method="post" enctype="multipart/form-data">
                                        <div class="form-group col-md-12 m-t-5">
                                            <label>Address</label>
                                            <textarea id="paraddress" name="paraddress" value="<?php if (!empty($permanent->address)) echo $permanent->address  ?>" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> class="form-control" rows="3" minlength="7" required><?php if (!empty($permanent->address)) echo $permanent->address  ?></textarea>
                                        </div>
                                        <div class="form-group col-md-6 m-t-5">
                                            <label>City</label>
                                            <input type="text" id="parcity" name="parcity" class="form-control form-control-line" placeholder="" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> value="<?php if (!empty($permanent->city)) echo $permanent->city ?>" minlength="2" required>
                                        </div>
                                        <div class="form-group col-md-6 m-t-5">
                                            <label>Country</label>
                                            <input type="text" id="parcountry" name="parcountry" class="form-control form-control-line" placeholder="" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> value="<?php if (!empty($permanent->country)) echo $permanent->country ?>" minlength="2" required>
                                        </div>
                                        <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?>
                                        <?php } else { ?>
                                            <div class="form-actions col-md-12">
                                    ...
                                    <div class="col-md-12">
                                        <input class="form-check-input" type="checkbox" value="" onchange="sameadr();" id="sameadr">
                                        <label class="form-check-label" for="sameadr">
                                            Same as above
                                        </label>
                                    </div>
                                    <hr>
                                    <form class="row" action="Present_Address" method="post" enctype="multipart/form-data">
                                        <div class="form-group col-md-12 m-t-5">
                                            <label>Address</label>
                                            <textarea id="presaddress" name="presaddress" value="<?php if (!empty($present->address)) echo $present->address  ?>" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> class="form-control" rows="3" minlength="7" required><?php if (!empty($present->address)) echo $present->address  ?></textarea>
                                        </div>
                                        <div class="form-group col-md-6 m-t-5">
                                            <label>City</label>
                                            <input type="text" id="prescity" name="prescity" class="form-control form-control-line" value="<?php if (!empty($present->address)) echo $present->city  ?>" placeholder=" City name" minlength="2" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> required>
                                        </div>
                                        <div class="form-group col-md-6 m-t-5">
                                            <label>Country</label>
                                            <input type="text" id="prescountry" name="prescountry" class="form-control form-control-line" placeholder="" value="<?php if (!empty($present->address)) echo $present->country  ?>" minlength="2" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?> readonly <?php } ?> required>
                                        </div>
                                        <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') { ?>
                                        <?php } else { ?>
                                            <div class="form-actions col-md-12">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="jsgrid-align-center ">
                                                            ...
                                                            <?php } else { ?>
                                                                ...
                                                                <a onclick="confirm('Are you sure want to delet this Value?')" href="#" title="Delete" class="btn btn-sm btn-danger waves-effect waves-light deletdoc" data-id="<?php echo $value->id ?>"><i class="fa fa-trash-o"></i></a>
                                                            <?php } ?>
                                                        </td>
                                                </tr>
                                            <?php endforeach; ?>
...
<script type="text/javascript">
        ...
        <script type="text/javascript">
            ...
            function sameadr(){
                var status=document.getElementById('sameadr').checked;

                if(status){
                    document.getElementById('presaddress').value=document.getElementById('paraddress').value;
                    document.getElementById('prescity').value=document.getElementById('parcity').value;
                    document.getElementById('prescountry').value=document.getElementById('parcountry').value;
                    $('#presaddress').attr('readonly','readonly');
                    $('#prescity').attr('readonly','readonly');
                    $('#prescountry').attr('readonly','readonly');

                }
                else
                {
                    document.getElementById('presaddress').value="";
                    document.getElementById('prescity').value="";
                    document.getElementById('prescountry').value="";
                    $('#presaddress').removeAttr('readonly');
                    $('#prescity').removeAttr('readonly');
                    $('#prescountry').removeAttr('readonly');

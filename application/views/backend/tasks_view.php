                                        ...
                                        <tr>
                                            ...
                                            <?php if ($this->session->userdata('user_type') != 'EMPLOYEE') {
                                               ...
                                               <td class="jsgrid-align-center ">
                                                    <a href="#" title="Edit" class="btn btn-sm btn-info waves-effect waves-light taskmodal"  data-toggle="modal" data-target="#exampleModal" data-id="' . $value->id . '"><i class="fa fa-pencil-square-o"></i></a>
                                                    <a  href="#" title="Delete" class="btn btn-sm btn-danger waves-effect waves-light TasksDelet"data-id="' . $value->id . '"><i class="fa fa-trash-o"></i></a>
                                                </td>';
                                            } else {
                                                echo ' <td class="jsgrid-align-center ">
                                                <a href="#" title="Edit" class="btn btn-sm btn-info waves-effect waves-light taskmodal"  data-toggle="modal" data-target="#exampleModal" data-id="' . $value->id . '"><i class="fa fa-pencil-square-o"></i></a>
            ...
            <div class="modal-content ">
                ...
                <div class="modal-header">
                    <?php if ($this->session->userdata('user_type') != 'EMPLOYEE') {
                        ...
                        echo ' <h4 class="modal-title" id="exampleModalLabel1">Task Details</h4>';
                    }
                    ?>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <select class="form-control custom-select col-md-8 proid" data-placeholder="Choose a Category" tabindex="1" name="projectid">
                               ...
                               <?php  if($this->session->userdata('user_type') == 'EMPLOYEE') {?>
                                    ...
                                    <label class="control-label col-md-3">Project List</label>
                                    <select class="form-control custom-select col-md-8 proid" data-placeholder="Choose a Category" tabindex="1" name="projectid">
                                        <?php foreach ($this->project_model->GetemployeeProjectsValue() as $value) : ?>
                                            <option value="<?php echo $value->id; ?>"><?php echo $value->pro_name; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                        ...
                        <?php if ($this->session->userdata('user_type') != 'EMPLOYEE') { ?>
                            <div class="form-group row">
                                ...
                                <select class="select2  custom-select col-md-3" data-placeholder="Choose a Category" style="width:25% " tabindex="1" name="teamhead">
                                ...
                                <select class="select2 select2-multiple col-md-3" data-placeholder="Choose a Category" multiple="multiple" style="width:25%" tabindex="1" name="assignto[]">
                                    <option value="">Select Here</option>
                                    <?php foreach ($employee as $value) : ?>
                                        <option value="<?php echo $value->em_id; ?>"><?php echo $value->first_name . ' ' . $value->last_name; ?></option>
                                    <?php endforeach; ?>
                        ...
                        <?php } else { ?>
                            <div class="form-group row" style="display:none;">
                                ...
                                <label class="control-label col-md-2">Collaborators</label>
                                <select class="select2 select2-multiple col-md-3" data-placeholder="Choose a Category" multiple="multiple" style="width:25%" tabindex="1" style="display:none;" name="assignto[]">
                                    <option value="">Select Here</option>
                                    <?php foreach ($employee as $value) : ?>
                                        <option value="<?php echo $value->em_id; ?>"><?php echo $value->first_name . ' ' . $value->last_name; ?></option>
                                    <?php endforeach; ?>
                        ...
                        <div class="form-group row">
                            ...
                            <input type="text" name="tasktitle" class="form-control col-md-8" id="recipient-name1" minlength="8" maxlength="250" placeholder="Task..." <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') {echo'readonly';}?>>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-md-3">Task Start Date</label>
                            <input type="text" name="startdate" class="form-control col-md-3 mydatetimepickerFull" id="recipient-name1" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') {echo'readonly';}?>>

                            <label class="control-label col-md-2">Task End Date</label>
                            <input type="text" name="enddate" class="form-control col-md-3 mydatetimepickerFull" id="recipient-name1" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') {echo'readonly';}?>>
                        </div>
                        <div class="form-group row">
                            <label for="message-text" class="control-label col-md-3">Details</label>
                            <textarea class="form-control col-md-8" name="details" id="message-text1" minlength="10" maxlength="1400" <?php if ($this->session->userdata('user_type') == 'EMPLOYEE') {echo'readonly';}?>></textarea>
                        </div>
                        <div class="form-group row">
                            <label class="control-label col-md-3">Status: </label>

                            <input name="status" type="radio" id="radio_4" class="type" value="open">
                            <label for="radio_4">Open</label>

                            <input name="status" type="radio" id="radio_1" class="type" value="complete">
                            <label for="radio_1">Complete</label>

                            <input name="status" type="radio" id="radio_2" class="type" value="running">
                            <label for="radio_2">Running</label>

                            <input name="status" type="radio" id="radio_3" class="type" value="cancel">
                            <label for="radio_3">Cancel</label>

                        </div>
                        <div class="form-group row" style="display:none" >
                            <label class="control-label col-md-3">Type: </label>
                            <input name="type" type="radio" id="radio_5" data-value="Logistic" class="type" value="Office" checked>
                            <label for="radio_5">Office</label>
                            <input name="type" type="radio" id="radio_6" data-value="Logistic" class="type" value="Field">
                            <label for="radio_6">Field</label>
                        </div>
                    </div>
        ...
        $(document).ready(function() {
            $(".taskmodal").click(function(e) {
                ...
                $.ajax({
                    ...
                    $('#tasksModalform').find('[name="teamhead"]').val(response.employesvalue[0].assign_user).end();
                    var collabs = [];
                    for (i = 1; i < response.employesvalue.length; i++) {
                        collabs.push(response.employesvalue[i].assign_user);
                    }

                    $('#tasksModalform').find('[name="assignto[]"]').val(collabs).end();
                    ...
                    if ($('#tasksModalform').find('[id="radio_1"]').val() == response.tasksvalue.status) {
                        $('#radio_1').attr('checked', 'true');
                    }
                    if ($('#tasksModalform').find('[id="radio_2"]').val() == response.tasksvalue.status) {
                        $('#radio_2').attr('checked', 'true');
                    }
                    if ($('#tasksModalform').find('[id="radio_3"]').val() == response.tasksvalue.status) {
                        $('#radio_3').attr('checked', 'true');
                    }
                    if ($('#tasksModalform').find('[id="radio_4"]').val() == response.tasksvalue.status) {
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $(".TasksDelet").click(function(e) {
                e.preventDefault(e);
                alert('Are You Want To Delete This Task!!!');
                // Get the record's ID via attribute  
                var iid = $(this).attr('data-id');
                $.ajax({

<?php
  ...
  public function checkattend($em_id)
  {
    $currdate=date("Y-m-d");

    $sql    = "SELECT * FROM attendance WHERE emp_id='$em_id' and atten_date= '$currdate'  ";
      $query  = $this->db->query($sql);
      $result = $query->num_rows();
      return $result;
  }

  public function getattend($em_id)
  {
    $currdate=date("Y-m-d");
  ...
  public function emselectforadmin(){
    $depid=$this->session->userdata('dep_id');
    $sql = "SELECT * FROM `employee` WHERE `status`='ACTIVE' AND em_role!='SUPER ADMIN' AND dep_id=$depid";
    $query=$this->db->query($sql);
  	$result = $query->result();
  	return $result;
	}
    ...
    $result=$this->db->insert('assign_leave',$data);
    $a=10;
  }
  ...
  {
    # code...
   return  $this->db->delete('employee',array('em_id'=>$id));
  }

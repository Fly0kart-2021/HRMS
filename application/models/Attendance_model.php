...
class Attendance_model extends CI_Model
{
    ...
    public function em_check_attendance($date)
    {
      $id=$this->session->userdata('user_login_id');
        $sql    = "SELECT `attendance`.*,
      `employee`.`em_id`,`first_name`,`last_name`,`em_code`
      FROM `attendance`
      LEFT JOIN `employee` ON `attendance`.`emp_id`=`employee`.`em_code` WHERE attendance.atten_date='$date' and (`attendance`.`status` = 'A' or `attendance`.`status` = 'E') and employee.em_id='$id' ORDER BY `attendance`.`id` DESC";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    public function timeout($em_id, $data)
    {
      $date=date('Y-m-d');

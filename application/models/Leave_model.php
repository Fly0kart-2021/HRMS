    ...
    public function GetEmLEaveReport($emid, $day, $year){
        ...
        $sql = "SELECT `emp_leave`.*,
                (SELECT SUM(`leave_duration`) 
                    ...
                    `employee`.`first_name`,`last_name`,`em_code`,`leave_status`,
                    `leave_types`.`name`
                FROM `emp_leave`
...
*/
    ...
    public function emEarnselectByLeave($emid){
        $sql = "SELECT * FROM `assign_leave` WHERE `emp_id`='$emid' and `type_id`=5";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
    ...
    $sql = "SELECT `emp_leave`.*,
      ...
      LEFT JOIN `employee` ON `emp_leave`.`em_id`=`employee`.`em_id`
      LEFT JOIN `leave_types` ON `emp_leave`.`typeid`=`leave_types`.`type_id`
      WHERE `emp_leave`.`em_id`='$emid' order by emp_leave.apply_date desc";
        $query=$this->db->query($sql);
		$result = $query->result();
		return $result; 
    }
    ...
    public function GetAllEmployeeLeaves()
    {
        # code...
        $sql = "SELECT `emp_leave`.*,
        `employee`.`em_id`,`first_name`,`last_name`,`em_code`,
        ...
        LEFT JOIN `employee` ON `emp_leave`.`em_id`=`employee`.`em_id`
        LEFT JOIN `leave_types` ON `emp_leave`.`typeid`=`leave_types`.`type_id`
        WHERE `emp_leave`.`leave_status`='Not Approve' AND  `employee`.`em_role`='EMPLOYEE'";
          $query=$this->db->query($sql);
          $result = $query->result();
          return $result; 
    }
 ...
 public function gettotalleave( $id = null)
 {
     # code...
     return $this->db->query("Select leave_day from leave_types where type_id=$id")->row();
 }

 public function getjoiningdate($employeeID)
 {
     # code...
     return $this->db->query("Select em_joining_date from employee where em_id='$employeeID'")->row();
 }

 public function leavestaken($fromdate,$todate,$emid,$lid)
 {
     # code...
     $sql="select SUM(leave_duration) as totalleave from emp_leave WHERE (start_date BETWEEN '$fromdate' AND '$todate') AND em_id='$emid' AND typeid=$lid AND (leave_status='Approve' or leave_status='Not Approve')";
     $query = $this->db->query($sql);
     return $query->row();
 }
 
 public function checkleaveforattend($date)
 {
     # code...
     $emid=$this->session->userdata('user_login_id');
     $sql="select * from emp_leave where (start_date='$date' or $date between start_date and end_date) and em_id='$emid' and leave_status='Approve'";
     $query = $this->db->query($sql);
     return $query->row();

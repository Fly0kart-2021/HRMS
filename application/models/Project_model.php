...
class Project_model extends CI_Model
{
    ...
    {
        $sql =
            "SELECT department.dep_name,project.id,project.pro_name,pro_start_date,pro_end_date,pro_description,pro_summary,pro_status,progress,dep_id FROM `project`  left join department on project.dep_id=department.id";
    ...
    //     $sql = "SELECT department.dep_name AS dname FROM `project` left join department on project.dep_id=department.id ";
    //     $query=$this->db->query($sql);
    //     $result = $query->result();
    // return $result;
    // }

    public function GetemployeeProjectsValue()
    {
        $uid = $this->session->userdata("user_login_id");
        $sql = "SELECT DISTINCT(project.id),pro_name FROM project LEFT JOIN assign_task ON assign_task.project_id=project.id WHERE assign_task.assign_user='$uid' ";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }

    public function GetadminProjectsValue()
    {
        $dept = $this->session->userdata("dep_id");
        $sql = "SELECT department.dep_name,project.id,project.pro_name,pro_start_date,pro_end_date,pro_description,pro_summary,pro_status,progress,dep_id FROM `project`  left join department on project.dep_id=department.id  where dep_id='$dept' ";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    public function GetEmProjectsValue($id, $depid)
    {
        ...
        $sql = "SELECT * FROM `project` WHERE `dep_id`='$depid'";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    public function GetAllTasksList($uid)
    {
        $sql = "SELECT `pro_task`.*, assign_task.task_id,`project`.`pro_name` FROM `pro_task` LEFT JOIN `project` ON `pro_task`.`pro_id`=`project`.`id` LEFT JOIN assign_task ON pro_task.id=assign_task.task_id WHERE assign_task.assign_user='$uid'";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }

    public function GetSAdminTasksList()
    {
        $sql = "SELECT `pro_task`.*,
        `project`.`pro_name`
        FROM `pro_task`
    ...
    {
        $this->db->delete("pro_task", ["id" => $id]);
    }
    public function DeletAssignuser($id)
    {
        $this->db->delete("assign_task", ["task_id" => $id]);
    }
    ...
    {
        $this->db->delete("project_file", ["id" => $id]);
    }
    public function DeletExpensesByid($id)
    {
        $this->db->delete("pro_expenses", ["id" => $id]);
    }
    public function DeletNotesByID($id)
    {
        $this->db->delete("pro_notes", ["id" => $id]);
    }
    ...
    {
        $this->db->delete("pro_notes", ["pro_id" => $id]);
        $this->db->delete("pro_task", ["pro_id" => $id]);
        $this->db->delete("project", ["id" => $id]);
        $this->db->delete("pro_expenses", ["pro_id" => $id]);
        $this->db->delete("project_file", ["pro_id" => $id]);
        $this->db->delete("assign_task", ["project_id" => $id]);
    }
    //Approve field application - update
    public function fieldVisitDoneAndUpdateAttendance(
        $fieldApplicationID,
        $data
    ) {
        $this->db->where("id", $fieldApplicationID);
    ...
    public function get_projectlist_by_dep($depid = null)
    {
        # code...
        $sql = "select id,pro_name from project where dep_id=$depid";

<?php
    ...
    public function getAllSalaryDataByMonthYearEm($eid,$month,$year){
      ...
      LEFT JOIN `employee` ON `pay_salary`.`emp_id`=`employee`.`em_id`
      LEFT JOIN `salary_type` ON `pay_salary`.`type_id`=`salary_type`.`id`
      WHERE `employee`.`em_id`='$eid' AND `pay_salary`.`month`='$month' AND `pay_salary`.`year`='$year' and pay_salary.status='Paid'";
        $query=$this->db->query($sql);
		$result = $query->row();
		return $result;        
    }
...
/*Invoice End*/   
    ...
    public function getAllSalaryData(){
      $sql = "SELECT `pay_salary`.*,
              `employee`.`first_name`,`last_name`,`em_code`,department.dep_name
              FROM `pay_salary`
              LEFT JOIN `employee` ON `pay_salary`.`emp_id`=`employee`.`em_id` LEFT JOIN department ON department.id = employee.dep_id
...
public function getPinFromID($employeeID){
    ...
    public function getattendbyid($mon,$year,$emid)
    {
      # code...
      $sql = "Select working_hour, atten_date from attendance left join employee on attendance.emp_id=employee.em_code where MONTH(atten_date)='$mon' and Year(atten_date)='$year' AND  employee.em_id='$emid'";
      $query=$this->db->query($sql);
     return $query->result();
    
    }
    public function getholidaylist( $month,$year)
    {
      # code...
      $sql="SELECT * FROM holiday WHERE YEAR(from_date)='$year' AND MONTH(from_date)='$month'";
      $query=$this->db->query($sql);
      return $query->result();
    }

    public function getmonthlypaidleave( $month,$year,$empid)
    {
      # code...
      $sql="SELECT * FROM emp_leave WHERE YEAR(start_date)='$year' AND MONTH(start_date)='$month' AND typeid=8 AND em_id='$empid' and leave_status='Approve'";
      $query=$this->db->query($sql);
      return $query->result();
    }

    public function getaddition($empid)
    {
      # code...
      $sql="SELECT addition.* FROM addition LEFT JOIN emp_salary ON  emp_salary.id=addition.salary_id WHERE emp_id='$empid'";
      $query=$this->db->query($sql);
      return $query->result();
    }

    public function getdeduction($empid)
    {
      # code...
      $sql="SELECT deduction.* FROM deduction LEFT JOIN emp_salary ON  emp_salary.id=deduction.salary_id WHERE emp_id='$empid'";
      $query=$this->db->query($sql);
      return $query->result();
    }

    public function getsalarypaydetail($empid)
    {
      # code...
      $sql="SELECT* FROM pay_salary WHERE emp_id='$empid'";
      $query=$this->db->query($sql);
      return $query->result();
    }

    public function finalpay($depid,$month,$year)
    {
      # code...
      $sql="UPDATE pay_salary LEFT JOIN employee ON employee.em_id=pay_salary.emp_id set pay_salary.STATUS ='Paid' where month='$month' AND YEAR=$year AND employee.dep_id=$depid";
      $query=$this->db->query($sql);
      return $query;
    }
}